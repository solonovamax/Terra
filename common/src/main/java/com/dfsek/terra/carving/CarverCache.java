        ...
        cache = CacheBuilder.newBuilder().maximumSize(main.getTerraConfig().getCarverCacheSize())
                            .build(new CacheLoader<Long, List<Worm.WormPoint>>() {
                                ...
                                public List<Worm.WormPoint> load(@NotNull Long key) {
                                    ...
                                    if(CarverCache.this.carver.isChunkCarved(w, chunkX, chunkZ, new FastRandom(
                                            MathUtil.getCarverChunkSeed(chunkX, chunkZ,
                                        ...
                                        for(int i = 0; i < carving.getLength(); i++) {
                                            ...
                                            if(!((UserDefinedBiome) biome).getConfig().getCarvers().containsKey(
                                                    CarverCache.this.carver)) { // Stop if we enter a biome this carver is not present in
                                                return new GlueList<>();
                                            }
