...
public class UserDefinedCarver extends Carver {
    ...
    public UserDefinedCarver(Range height, Range length, double[] start, double[] mutate, List<String> radii, Scope parent, long hash,
                             int topCut, int bottomCut, CarverTemplate config, TerraPlugin main, Map<String, NoiseSeeded> functions,
                             Map<String, FunctionTemplate> definedFunctions) throws ParseException {
    ...
    public boolean isChunkCarved(World w, int chunkX, int chunkZ, Random random) {
        BiomeTemplate conf = ((UserDefinedBiome) main.getWorld(w).getBiomeProvider().getBiome((chunkX << 4) + 8,
                                                                                              (chunkZ << 4) + 8)).getConfig();
