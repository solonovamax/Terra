...
public class ReplaceListMutatorTemplate extends MutatorStageTemplate {
    ...
    public BiomeMutator build(long seed) {
        ...
        replace.forEach((biomeBuilder, biomeBuilders) -> replaceMap.put(biomeBuilder.apply(seed),
                                                                        biomeBuilders.map(builder -> builder.apply(seed), true)));
        
        return new ReplaceListMutator(replaceMap, defaultFrom, defaultTo.map(biomeBuilder -> biomeBuilder.apply(seed), true),
                                      noise.apply(seed));
    }
}
