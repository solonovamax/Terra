...
public class ChunkInterpolator3D implements ChunkInterpolator {
    ...
    public ChunkInterpolator3D(World w, int chunkX, int chunkZ, BiomeProvider provider,
                               BiFunction<Generator, Vector3, Double> noiseGetter) {
                ...
                for(int xi = -blend; xi <= blend; xi++) {
                    for(int zi = -blend; zi <= blend; zi++) {
                        genMap.computeIfAbsent(
                                provider.getBiome(xOrigin + (x << 2) + (xi * step), zOrigin + (z << 2) + (zi * step)).getGenerator(w),
    ...
    public double getNoise(double x, double y, double z) {
        return interpGrid[reRange(((int) x) / 4, 3)][FastMath.max(FastMath.min(((int) y), max), min) / 4][reRange(((int) z) / 4,
                                                                                                                  3)].trilerp((x % 4) / 4,
                                                                                                                              (y % 4) / 4,
                                                                                                                              (z % 4) / 4);
    }
