...
public class TerraFlora implements Flora {
    ...
    public TerraFlora(Palette floraPalette, boolean physics, boolean ceiling, MaterialSet irrigable, MaterialSet spawnable,
                      MaterialSet replaceable, MaterialSet testRotation, int maxPlacements, Search search, boolean spawnBlacklist,
                      int irrigableOffset, TerraPlugin main) {
    ...
    public boolean plant(Location location) {
        ...
        for(int i = 0; FastMath.abs(i) < size; i += c) { // Down if ceiling, up if floor
            ...
            if(doRotation) {
                BlockFace oneFace = faces.get(
                        new FastRandom(location.getBlockX() ^ location.getBlockZ()).nextInt(faces.size())); // Get random face.
    ...
    public List<Block> getValidSpawnsAt(Chunk chunk, int x, int z, Range range) {
        ...
        for(int y : range) {
            ...
            if((spawnBlacklist != spawnable.contains(current.getType())) && isIrrigated(
                    current.getRelative(BlockFace.UP, irrigableOffset)) && valid(size, current)) {
                blocks.add(current);
