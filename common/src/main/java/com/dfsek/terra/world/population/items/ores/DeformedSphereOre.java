...
public class DeformedSphereOre extends Ore {
    ...
    public DeformedSphereOre(BlockData material, MaterialSet replaceable, boolean applyGravity, double deform, double deformFrequency,
                             Range size, TerraPlugin main) {
        ...
        for(int x = -rad; x <= rad; x++) {
            for(int y = -rad; y <= rad; y++) {
                for(int z = -rad; z <= rad; z++) {
                    ...
                    if(oreLoc.getBlockX() > 15 || oreLoc.getBlockZ() > 15 || oreLoc.getBlockY() > 255 || oreLoc.getBlockX() < 0 ||
                       oreLoc.getBlockZ() < 0 || oreLoc.getBlockY() < 0)
                        continue;
