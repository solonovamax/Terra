...
public class BukkitMultipleFacing extends BukkitBlockData implements MultipleFacing {
    ...
    public Set<BlockFace> getAllowedFaces() {
        return ((org.bukkit.block.data.MultipleFacing) super.getHandle()).getAllowedFaces().stream().map(BukkitAdapter::adapt).collect(
                Collectors.toSet());
    }
    
    @Override
    public Set<BlockFace> getFaces() {
        return ((org.bukkit.block.data.MultipleFacing) super.getHandle()).getFaces().stream().map(BukkitAdapter::adapt).collect(
                Collectors.toSet());
    }
