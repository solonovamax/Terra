...
public class ForgeChunkGeneratorWrapper extends ChunkGenerator implements GeneratorWrapper {
    ...
    public static final Codec<ConfigPack> PACK_CODEC = RecordCodecBuilder.create(config -> {
        return config.group(Codec.STRING.fieldOf("pack").forGetter(pack -> pack.getTemplate().getID()))
                     .apply(config, config.stable(TerraForgePlugin.getInstance()
                                                                  .getConfigRegistry()::get));
    });
    @SuppressWarnings("CodeBlock2Expr")
    public static final Codec<ForgeChunkGeneratorWrapper> CODEC = RecordCodecBuilder.create(instance -> {
        return instance.group(TerraBiomeSource.CODEC.fieldOf("biome_source")
                                                    .forGetter(generator -> generator.biomeSource),
                              Codec.LONG.fieldOf("seed")
                                        .stable()
                                        .forGetter(generator -> generator.seed),
                              PACK_CODEC.fieldOf("pack")
                                        .stable()
