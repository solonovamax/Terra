...
public class DirectBlock implements Block {
    ...
    public void setBlockData(BlockData data, boolean physics) {
        synchronized(world) {
            world.compute(FastMath.floorDiv(pos.getBlockX(), 16), FastMath.floorDiv(pos.getBlockZ(), 16)).setBlockStateAt(pos.getBlockX(),
                                                                                                                          pos.getBlockY(),
                                                                                                                          pos.getBlockZ(),
                                                                                                                          ((Data) data).getHandle(),
                                                                                                                          false);
    ...
    public Block getRelative(BlockFace face, int len) {
        return world.getBlockAt(pos.getBlockX() + face.getModX() * len, pos.getBlockY() + face.getModY() * len,
                                pos.getBlockZ() + face.getModZ() * len);
    }
